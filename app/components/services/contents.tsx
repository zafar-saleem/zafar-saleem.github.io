export const contents = [
  {
    title: "User Interface Development",
    desc: "Need stunning and engaging UIs?",
    icon: "data:image/svg+xml;base64,",
    contents: [
      "I build responsive user interfaces for your websites",
    ],
    action: {
      label: "Contact me",
      url: "mailto:zafarsaleem3@gmail.com?subject=I would like to hire you for [enter project name/position]",
      classes: "bg-amber-600 text-white hover:bg-orange-500 hover:text-white",
    }
  },
  {
    title: "Dashboard Development",
    desc: "Do you need help with your dashboards?",
    icon: "data:image/svg+xml;base64,",
    contents: [
      "I build dashboards for your SaaS Web Portals",
    ],
    action: {
      label: "Contact me",
      url: "mailto:zafarsaleem3@gmail.com?subject=I would like to hire you for [enter project name/position]",
      classes: "bg-sky-600 text-white hover:bg-sky-500 hover:text-white",
    }
  },
  {
    title: "E-Commerce Development",
    desc: "Don't have e-commerce website?",
    icon: "data:image/svg+xml;base64,",
    contents: [
      "I build SEO optimized e-commerce websites for you",
    ],
    action: {
      label: "Contact me",
      url: "mailto:zafarsaleem3@gmail.com?subject=I would like to hire you for [enter project name/position]",
      classes: "bg-emerald-600 text-white hover:bg-emerald-500 hover:text-white",
    }
  },
  {
    title: "Web Portal Development",
    desc: "Do you need help in web portal?",
    icon: "data:image/svg+xml;base64,",
    contents: [
      "I build web portal for you next big thing",
    ],
    action: {
      label: "Contact me",
      url: "mailto:zafarsaleem3@gmail.com?subject=I would like to hire you for [enter project name/position]",
      classes: "bg-sky-600 text-white hover:bg-sky-500 hover:text-white",
    }
  }
];
